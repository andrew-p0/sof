...
static const struct mtk_base_memif_data memif_data[MT8168_MEMIF_NUM] = {
	[MT8168_MEMIF_DL1] = {
		.name = "DL1",
		.id = MT8168_MEMIF_DL1,
...
static const struct mt8168_afe_rate mt8168_afe_rates[] = {
	{
		.rate = 8000,
		.reg_value = 0,
	},
...
static unsigned int mt8168_afe_fs_timing(unsigned int rate)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(mt8168_afe_rates); i++)
		if (mt8168_afe_rates[i].rate == rate)
			return mt8168_afe_rates[i].reg_value;

	return -EINVAL;
}

static unsigned int mt8168_afe_fs(unsigned int rate, int aud_blk)
{
	return mt8168_afe_fs_timing(rate);
}

static unsigned int mt8168_afe2adsp_addr(unsigned int addr)
{
	/*TODO : Need apply the address remap */
	return addr;
}

static unsigned int mt8168_adsp2afe_addr(unsigned int addr)
{
	/* TODO : Need apply the address remap */
	return addr;
}
...
struct mtk_base_afe_platform mtk_afe_platform = {
	...
	.memif_size = MT8168_MEMIF_NUM,
	.memif_dl_num = MT8168_MEMIF_DL_NUM,
	.memif_32bit_supported = 0,
	.irq_datas = NULL,
	.irqs_size = 0,
	.dais_size = MT8168_DAI_NUM,
	.afe2adsp_addr = mt8168_afe2adsp_addr,
	.adsp2afe_addr = mt8168_adsp2afe_addr,
	.afe_fs = mt8168_afe_fs,
	.irq_fs = mt8168_afe_fs_timing,
};
