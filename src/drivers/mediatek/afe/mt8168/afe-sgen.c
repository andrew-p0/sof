...
#include <mt8168-afe-regs.h>
#include <mt8168-afe-common.h>

SOF_DEFINE_REG_UUID(sgen_mt8168);

DECLARE_TR_CTX(sgen_tr, SOF_UUID(sgen_mt8168_uuid), LOG_LEVEL_INFO);
 ...
 * e.g. set TEST_SGEN as '1' and TEST_SGEN_ID as "MT8168_MEMIF_DL1",
 * the data source of DL2 will from sine generator.
 */
#define TEST_SGEN_ID MT8168_MEMIF_DL1
...
enum {
	MT8168_SGEN_AWB = (0x3 << 1) | 1,		// o5 + o6 + SGEN to out
	MT8168_SGEN_VUL = (0x5 << 1) | 1,		// o9 + o10 + SGEN to out
	MT8168_SGEN_DL1 = (0x3 << 1) | 0,		// i5 + i6 + SGEN to in
	MT8168_SGEN_DL2 = (0x4 << 1) | 0		// i7 + i8 + SGEN to in
};
...
/*
static uint32_t mt8168_sinegen_timing(uint32_t rate)
{
...
static void mt8168_afe_sinegen_enable(uint32_t sgen_id, uint32_t rate, int enable)
{
	uint32_t loopback_mode, reg_1;
//, sinegen_timing;

	tr_dbg(&sgen_tr, "sgen_id %d, enable %d", sgen_id, enable);

//	sinegen_timing = mt8168_sinegen_timing(rate);

	if (enable == 1) {
		/* set loopback mode */
		switch (sgen_id) {
		case MT8168_MEMIF_AWB:
			loopback_mode = MT8168_SGEN_AWB;
			break;
		case MT8168_MEMIF_VUL:
			loopback_mode = MT8168_SGEN_VUL;
			break;
		case MT8168_MEMIF_DL1:
			loopback_mode = MT8168_SGEN_DL1;
			break;
		case MT8168_MEMIF_DL2:
			loopback_mode = MT8168_SGEN_DL2;
			break;
		default:
...
{
	mt8168_afe_sinegen_enable(TEST_SGEN_ID, 48000, 1);
}

void afe_sinegen_disable(void)
{
	mt8168_afe_sinegen_enable(TEST_SGEN_ID, 48000, 0);
}
